...
function AttributeAllocation({ attributes, availablePoints, onIncrease, onDecrease }: AttributeAllocationProps) {
  const generalAttributes: Array<{ key: keyof Attributes; name: string }> = [
  ...
  const specializedAttributes: Array<{ key: keyof Attributes; name: string }> = [
    { key: 'math', name: '📐 数学' },
    { key: 'dp', name: '🔄 动态规划' },
    { key: 'graph', name: '🕸️ 图论' },
  ...
  const renderAttributeGroup = (attributeList: Array<{ key: keyof Attributes; name: string }>, title: string) => (
    <AttributeGroup>
      <h3>{title}</h3>
      <AttributesGrid>
